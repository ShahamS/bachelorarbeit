"""Ergebnisobjekte fuer zentrales Token-Based Replay."""

from dataclasses import dataclass, field


@dataclass
class CentralReplayEventResult:
    case_id: str
    activity: str
    matched: bool
...
@dataclass
class CentralReplayTraceResult:
    case_id: str
    events: list[CentralReplayEventResult] = field(default_factory=list)
    remaining_tokens: int = 0
    missing_final_tokens: int = 0
    ...
    def missing_tokens(self):
        return sum(event.missing_tokens for event in self.events) + self.missing_final_tokens


@dataclass
class CentralReplayLogResult:
    traces: list[CentralReplayTraceResult] = field(default_factory=list)
    ...
    def total_consumed_tokens(self):
        return sum(trace.consumed_tokens for trace in self.traces)

    @property
    def total_produced_tokens(self):
        return sum(trace.produced_tokens for trace in self.traces)
    
    @property
    def total_missing_tokens(self):
        return sum(trace.missing_tokens for trace in self.traces)

    @property
    def total_remaining_tokens(self):
        return sum(trace.remaining_tokens for trace in self.traces)
